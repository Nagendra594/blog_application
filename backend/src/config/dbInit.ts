...
export const createTables = async (): Promise<void> => {
  try {
    ...
    const blogsTable =
      "CREATE TABLE IF NOT EXISTS blogs(id int AUTO_INCREMENT PRIMARY KEY, userId int NOT NULL, title VARCHAR(255) NOT NULL, content TEXT NOT NULL,image VARCHAR(255), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY (userId) REFERENCES users(id))";
